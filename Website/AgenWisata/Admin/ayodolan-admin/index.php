  ...
  <body>
      ...
      <div id="content" class="p-4 p-md-5">
        ...
        <!-- List Tabel -->
          ...
          <?php
            require('config.php');
            $sql = "SELECT * FROM pemesanan JOIN pembayaran USING (id_pemesanan)";
            $result = mysqli_query($conn, $sql);
            if($result->num_rows > 0) {
                ...
                while($row = $result -> fetch_assoc()){
                    ...
                    <td>" . $row["bukti"]. "</td>
                    <td> 
                    <a href='edit.php?target=".$row["id_pelanggan"]."'> <i class='fa fa-pencil-square-o'></i>
                    <a href=hapus.php?id=".$row["id_pelanggan"]." onclick=\"return confirm('Apakah anda yakin untuk menghapus?')\"><i class='fa fa-trash-o'></i></a>
                     </td>
                 </tr>";
                  }
                echo "</table></div>";
